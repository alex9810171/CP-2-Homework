...
#include<string.h>
#include<math.h>
int num,i,j,k,l,m,n,brk,ntemp,table[30][37]={0},like[30][16]={0},like2[30][16],time[30];/*0=sex 1-20=style point 21-36=set*/
char temp,stemp[30],name[30][11],sex[7],pre[30][3][30],style[20][100];
void test(){
	printf("num=%d\n",num);
	printf("\nname:\n");
...
int main(){
	...
	for(i=0;i<num;i++){
		...
		temp='\0';
		for(j=0;j<20;j++)memset(style[j],'\0',30);/*clean style*/
		for(j=0;j<20&&temp!='\n';j++)scanf("%s %d%c",style[j],&table[i][j+1],&temp);/*scan*/
		for(j=0;j<19;j++)
			for(k=0;k<19-j;k++){
				brk=0;
				for(l=0;l<30&&brk==0;l++){
					if(style[k][l]>style[k+1][l]||(style[k][l]!='\0'&&style[k+1][l]=='\0')){
						memset(stemp,'\0',30);
						strcpy(stemp,style[k+1]);
						memset(style[k+1],'\0',30);
						strcpy(style[k+1],style[k]);
						memset(style[k],'\0',30);
						strcpy(style[k],stemp);/*exchange style*/
						ntemp=table[i][k+2];
						table[i][k+2]=table[i][k+1];
						table[i][k+1]=ntemp;/*exchange nstyle*/
						brk=1;
					}
					else if(style[k][l]<style[k+1][l]||(style[k][l]!='\0'&&style[k+1][l]=='\0'))brk=1;
				}
			}/*bb sort for style*/
	}/*input file*/
	for(i=0;i<num-1;i++)
	...
	for(i=0;i<num/2;i++)/*men*/
		for(j=num/2;j<num;j++){/*women*/
			for(l=0;l<3;l++)
				for(k=0;k<20;k++)
					if(strcmp(pre[i][l],style[k])==0)table[i][21+j-num/2]+=table[j][1+k]*pow(1000,(2-l));/*style*/
		}/*calculate score for men*/
	for(i=num/2;i<num;i++)/*women*/
		for(j=0;j<num/2;j++){/*men*/
			for(l=0;l<3;l++)
				for(k=0;k<20;k++)
					if(strcmp(pre[i][l],style[k])==0)table[i][21+j]+=table[j][1+k]*pow(1000,(2-l));
	...
	test();
	for(i=0;i<30;i++)
		for(j=0;j<16;j++)like2[i][j]=like[i][j];
	for(i=0;i<num/2;i++){
